...
void landMsg(int msg, std::ostream &cout)
{
    ...
    case 22:
        ...
        break;
    case 23:
        cout << "NASA WILL DEDUCT $" << msgarg << " FROM YOUR PAYCHECK EACH WEEK ";
        break;
    case 24:
...
struct LEMState {
    ...
    void P4250(double rate)
    {
        ...
        // newVmps = (((lunarG * deltaT) + Vmps) + (log(1.0-x) * impulse));
        newalt =
            (((altitude - (((lunarG * deltaT) * deltaT) / (2.0))) - (Vmps * deltaT)) +
             ((impulse * deltaT) *
              (((((x / (2.0)) + (x2 / (6.0))) + (x3 / (12.0))) + (x4 / (20.0))) + (x5 / (30.0)))));
    }
    ...
    {
        cout << std::setw(5) << round(elapsed)
             << std::setw(13) << trunc(altitude)
             << std::setw(8) << trunc((((altitude - (trunc(altitude))) * (5280)) + (0.5)))
             << std::setw(9) << trunc((((5280) * Vmps) + (0.5)))
             << std::setw(13) << round(((mass)-drymass));
        if (verbose)
            cout << ' ' << std::setw(0) << Vmps * Vmps / altitude;
        cout << '\n';
    }
...
void LEMState::landing(double fuel, std::istream &cin, std::ostream &cout)
{
    ...
    L4410:
        ...
        if (Vfps <= 3) {
            landMsg(17, cout); /* perfect */
        } else if (Vfps <= 15) {
            landMsg(18, cout); /* good */
        } else if (Vfps <= 35) {
            landMsg(19, cout); /* no Buck Rogers */
        } else if (Vfps <= 80) {
            landMsg(20, cout); /* heavy damage */
        } else {
...
bool LEMState::run1sec(double rate)
{
    remaining = 1.0;
L4472:
    if (mass - drymass < 1.0e-3) {
        ...
        L4636:
            ...
            deltaT = ((2.0 * altitude) /
                      (sqrt(((Vmps * Vmps) +
                             ((2.0 * altitude) * (lunarG - (((impulse)*rate) / mass))))) +
                       Vmps));
            P4250(rate);
            update(rate);
            goto L4636;
        ...
        L4664:
            ...
            deltaT = (((mass * Vmps) /
                       ((impulse * rate) * (sqrt(((Vfps * Vfps) + (Vmps / impulse))) + Vfps))) +
                      0.05);
            P4250(rate);
            if (newalt <= 0.0)
                goto L4636;
            update(rate);
            if ((-newVmps > 0.0) && (Vmps > 0.0)) {
            } else
                goto L4472;
            goto L4664;
        } else {
